...
pygame.display.set_caption("Space Noobie")
...
#SOUND
BULLET_HIT_SOUND = pygame.mixer.Sound(os.path.join('Assets','flashbang-1.wav'))
BULLET_FIRE_SOUND = pygame.mixer.Sound(os.path.join('Assets','mixkit-laser-cannon-shot-1678.wav'))
...
RED_SPACESHIP = pygame.transform.rotate(pygame.transform.scale(RED_SPACESHIP,(55,40)),-90)
...
def yellow_handle_movements(keys_pressed, yellow):
    ...
    if(keys_pressed[pygame.K_s]) and yellow.y+VEL+yellow.height < HEIGHT-12 : #DOWN KEY
        yellow.y += VEL
...
def red_handle_movements(keys_pressed, red):
    ...
    if(keys_pressed[pygame.K_DOWN]) and red.y+VEL+red.height < HEIGHT-12: #DOWN KEY
        red.y += VEL
